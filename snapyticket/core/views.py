...
def  payment_sucessful(request, order_id):
    ...
    user_ticket_bag = TicketBag.objects.get(user=request.user,ordered=False, order_ref_code=order_id)
    ...
    if user_ticket_bag.ordered == True:
        ...
        for ticketitems in user_ticket_bag.tickets.all():
            ...
            while ticketitems.ticketitemqrimage_set.count() < ticketitems.quantity:
                ...
                ticket_item_img.save(settings.MEDIA_ROOT+f'/qr_codes/{request.user}{counter}{ticketitems.ticket_code}.png')
                # open saved image
                with open(settings.MEDIA_ROOT+f'/qr_codes/{request.user}{counter}{ticketitems.ticket_code}.png', "rb") as ticket_qr:
                    # endcodes it
